...
server::server(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::server)
    , server_socket(new QTcpServer(this)) //创建初始化Socket对象
{
    ui->setupUi(this);
...
//    for (const struct ID_IP &clientID_IP : connectedClients) {
//        // 获取当前客户端的socket
//        for (QTcpSocket *clientSocket : server_socket->findChildren<QTcpSocket *>())
//        //遍历树 找到socket
//        {
//            if (clientSocket->peerAddress().toString() == clientID_IP.ip) {
//                // 发送消息
...
void server::onNewConnection()
{
    ...
    connect(clientSocket, &QTcpSocket::disconnected, this, [this, clientSocket]() {
        ...
        auto it = std::find_if(connectedClients.begin(), connectedClients.end(),
            [clientSocket](const ID_IP &user) { return user.socket == clientSocket; });
...
//        // 创建该用户
//        struct ID_IP user = ID_IP(clientId, clientIp);
...
//    // ----------------------------连接断开时的处理---------------------------------------
//    connect(clientSocket, &QTcpSocket::disconnected, this, [this, clientSocket, clientIp]() {
//        // 获取客户端的IP地址
//        // 从集合中移除客户端
//        auto it = std::find_if(connectedClients.begin(), connectedClients.end(),
//            [clientIp](const ID_IP &user) { return user.ip == clientIp; });
...
//        // 更新列表和状态
//        updateShowClientsLists();
//        ui->ShowSituation->append("客户端已断开连接: " + clientIp);
